    ...
    def forward(self, batch, run_vqa, run_ref, train):
        ...
        if run_ref:
            ...
            if train:
                ...
                current_box_mask = torch.zeros((batchSize, num_beams, num_obj), dtype=int).cuda()
                #output_box_inds = 
                output_box_mask = torch.zeros((batchSize, num_obj), dtype=int).cuda()
                ...
                for k in range(num_beams):
                    current_box_mask[torch.arange(batchSize, dtype=int), k, k_new_box_inds[:,k]] = 1
                #print('current_box_inds: ', current_box_inds.shape)
                current_set_feat[:,:,:] = x_out[torch.arange(batchSize, dtype=int),k_new_box_inds,:]
                ...
                while len(unfinished) > 0 and num_rounds < max_num_rounds:
                    ...
                    for k in range(num_beams):
                        new_set_feat = (1./num_rounds) * ((current_set_feat[:,k,:].unsqueeze(dim=1).expand((-1,num_obj,-1))) + x_out)
                        ...
                        if increase_inds.shape[0] > 0:
                            ...
                            #we need where current_box_inds[0] = increase_inds[:,0]
                            current_box_mask[increase_inds[:,0], increase_inds[:,1], :] = (current_box_mask[increase_inds[:,0], k, :])
                            current_set_feat[increase_inds[:,0],increase_inds[:,1],:] = new_set_feat[increase_inds[:,0],increase_inds[:,1],:]
                            #[torch.arange(batchSize, dtype=int).unsqueeze(dim=1).expand((-1,num_beams)), k_new_box_inds,:]
                    ...
                    output_box_maski[unfinished_decrease_inds, :] = current_box_mask[unfinished_decrease_inds, max_1_inds[unfinished_decrease_inds],:]
                    ...
                    for k in range(num_beams):
                        current_box_mask[unfinished_increase_inds, k, k_new_box_inds[:,k]] = 1 
                    num_rounds += 1
                    print('')
                ...
                if len(unfinished >=0):
                    ...
                    if max_num_rounds > 1 and num_beams > 1:
                        output_box_mask[unfinished_increase_inds,:] = current_box_inds[unfinished_increase_inds, max_1_inds[unfinished_increase_inds], :]
                    else:
                        output_box_mask = current_box_inds_mask
                        print('output_box_mask: ', output_box_inds.shape)
                set_box_inds = (output_box_mask == 1).nonzero()
                print('eval')
